...
const categories = {
  ...
  'pareti-componibili': { label: 'Pareti componibili', products: pareti.filter(p => ['FOSTER COMPONIBILI E MADIE','MERIDA','FORMENTERA','TAGO','BILBAO','MADRID','TOLEDO','NELSON','VIGO','CORDOVA','GRANADA'].includes(p.title)) },
  'pareti-bloccate': { label: 'Pareti bloccate', products: pareti.filter(p => ['ARGO','OMEGA','IBIZA','LEON','NEWMASTER','ALEXANDRA'].includes(p.title)) },
};

const paretiDir = path.join(ROOT, 'pareti-attrezzate');
await fs.mkdir(paretiDir, { recursive: true });

const detailTemplate = (p, slug, images, desc) => `<!DOCTYPE html>
<html lang="it">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${p.title} | Pareti attrezzate Soft Comfort</title>
  ...
  <main class="detail-shell">
    ...
    <section class="detail-hero">
      ...
      <div class="detail-copy">
        <p class="category">Pareti attrezzate</p>
        <h1>${p.title}</h1>
        <h2>${p.dimensions || 'Design contemporaneo'}</h2>
        <p class="description">${escapeHtml(desc || 'Parete attrezzata selezionata da Soft Comfort. Dettagli, finiture e disponibilità sono da confermare in showroom.')}</p>
        <div class="detail-actions">
          <a class="btn" href="https://wa.me/393929952453?text=Salve%20Soft%20Comfort%2C%20vorrei%20ricevere%20maggiori%20informazioni%20su%20questa%20parete%20attrezzata%3A%20${encodeURIComponent(p.title)}" target="_blank" rel="noopener">Richiedi informazioni</a>
          <a class="btn btn-secondary" href="../catalogo-pareti-attrezzate.html#product-pareti-${slug}">Torna al catalogo</a>
        </div>
      </div>
    </section>
${images.length > 1 ? `    <section class="gallery-section">
...
${images.slice(0, 8).map((img, i) => `          <button type="button" data-gallery-thumb="${img}" class="${i === 0 ? 'is-active' : ''}">
    ...
    <section class="detail-info"><p>${escapeHtml(desc || 'Parete attrezzata selezionata da Soft Comfort. Dettagli, finiture e disponibilità sono da confermare in showroom.')}</p></section>
  </main>
  <script>
...
for (const [catSlug, cat] of Object.entries(categories)) {
  for (const p of cat.products) {
    ...
    paretiCards.push({ catSlug, catLabel: cat.label, slug, title: p.title, cover: images[0] || '', desc: (desc || '').substring(0, 120) + ((desc || '').length > 120 ? '...' : ''), dimensions: p.dimensions || '' });
  }
}

console.log('Pareti cards:', paretiCards.length);

// Save cards data for catalog generation
await fs.writeFile(path.join(ROOT, 'premobil-data', 'pareti-cards.json'), JSON.stringify(paretiCards, null, 2), 'utf8');
